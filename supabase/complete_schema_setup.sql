-- COMPLETE DB SETUP SCRIPT FOR NEW SUPABASE PROJECT
...
DROP POLICY IF EXISTS "Own memory tags" ON public.memory_tags;
...
DROP POLICY IF EXISTS "Own memory relationships" ON public.memory_relationships;
...
DROP POLICY IF EXISTS "Own logs" ON public.app_logs;
...
DROP POLICY IF EXISTS "Own google tokens" ON public.user_google_tokens;
CREATE POLICY "Own google tokens" ON public.user_google_tokens FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP TRIGGER IF EXISTS user_google_tokens_updated_at ON public.user_google_tokens;
CREATE TRIGGER user_google_tokens_updated_at BEFORE UPDATE ON public.user_google_tokens FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
